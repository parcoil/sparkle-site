  ...
  useEffect(() => {
    async function fetchApps() {
      try {
        ...
        if (!Array.isArray(data)) throw new Error("Expected an array of apps");

        setApps(data);

        const newCategories = new Set(["all"]);
        data.forEach((app: App) => {
          if (app.category) newCategories.add(app.category);
        });
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {Array.from({ length: 8 }).map((_, idx) => (
            <div
              key={idx}
              className="h-40 animate-pulse rounded-xl bg-muted/30 p-6"
            >
